...
template< class TYPE = int >
  class Matrix
  {
    ...
    public:
      ...
      void operator% (TYPE const& mod) {

          for (unsigned row = 0; row < rows; ++row) {
              for (unsigned column = 0; column < columns; ++column) {
                  matrix[row][column] %= mod;
              }
          }

      }

      std::vector<TYPE> getRowAsVec(unsigned const int index) {
          return matrix[index];
      }

      void setRow(unsigned const int index, std::vector<TYPE> row) {
          matrix[index] = row;
      }
